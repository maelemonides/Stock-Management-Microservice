...
def move():
    ...
    #----------------------------------------------------------
    prod_choices = [product.prod_name for product in pdetails] #Product.query.with_entities(Product.prod_name, Product.prod_name).all()
...
def check(frm,to,name,qty):
    if frm == to :
        ...
        logging.debug(f"Product info transf : {prodq}")
